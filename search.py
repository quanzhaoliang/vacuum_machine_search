...
class Problem:
    ...
    def __init__(self, initial, direction,goal=None, ):
...
class Node:
    ...
    def __init__(self, state, direction, parent=None, action=None, path_cost=0):
    ...
    def child_node(self, problem, action):
        """[Figure 3.10]"""
        next_state, next_dir = problem.result(self.state, action)
        next_node = Node(next_state, next_dir, self, action, problem.path_cost(self.path_cost, self.state, action, next_state))
        return next_node
...
def breadth_first_graph_search(problem):
    ...
    """
    node = Node(problem.initial, problem.direction)  # FIFO queue
    if problem.goal_test(node.state):
        return node
    frontier = deque([node])
    explored = set()
...
def generate_hashcode(state, direction):
    arrow_codes = {
        'left': '<',
        'right': '>',
        'up': '^',
    ...
    x, y = state
    arrow_code = arrow_codes.get(direction, '?')
    hashcode = f"{x},{y}-{arrow_code}"
    return hashcode
def breadth_first_graph_search_TURN(problem):
    """[Figure 3.11]
    Note that this function can be implemented in a
    single line as below:
    return graph_search(problem, FIFOQueue())
    ...
    frontier = deque([node])
    explored = set()
    explored.add(generate_hashcode(node.state, node.direction))  # Add hashcode of initial state
    while frontier:
        node = frontier.popleft()
        explored.add(generate_hashcode(node.state, node.direction))
        for child in node.expand(problem):
            child_hashcode = generate_hashcode(child.state, child.direction)
            if child_hashcode not in explored and child not in frontier:
                if problem.goal_test(child.state):
                    return child, explored
                frontier.append(child)
    return None
...
def depth_first_graph_search(problem):
    ...
    """
    node = Node(problem.initial, problem.direction)# Stack
    if problem.goal_test(node.state):
        return node
    frontier = [node]
    explored = set()
...
def best_first_graph_search(problem, f, display=False):
    ...
    f = memoize(f, 'f')
    node = Node(problem.initial, problem.direction)
...
def uniform_cost_search(problem):
    ...
    while not frontier.empty():
        ...
        for action in problem.actions(node.state):
            child_state, child_dir = problem.result(node.state, action)
            child_cost = problem.path_cost(node.path_cost, node.state, action, child_state)
            child = Node(child_state, child_dir, node, action, child_cost)
            if tuple(child.state) not in explored and child not in frontier_set:
...
def astar_search(problem, h=None, display=False):
    ...
    if depth_first_graph_search(problem) is not None:
        p, e = depth_first_graph_search(problem)
    else:
        return None
    goalState = p.state
    node = Node(problem.initial, problem.direction)
    if problem.goal_test(node.state):
        return node
    frontier = queue.PriorityQueue()
    frontier.put((problem.h(node.state, goalState), node))
    frontier_set = set([node])
    explored = set()
    while not frontier.empty():
        _, node = frontier.get()
        ...
        for action in problem.actions(node.state):
            child_state, child_dir = problem.result(node.state, action)
            child_cost = problem.path_cost(node.path_cost, node.state, action, child_state) + problem.h(child_state, goalState)
            child = Node(child_state, child_dir, node, action, child_cost)
            if tuple(child.state) not in explored and child not in frontier_set:
                frontier.put((child.path_cost, child))
                frontier_set.add(child)
